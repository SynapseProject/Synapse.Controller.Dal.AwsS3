...
public partial class AwsS3Dal : IControllerDal
{
    static void Main(string[] args)
    {
        ...
        string[] parts = System.IO.File.ReadAllText( "creds.ignore" ).Split( ',' );

        AwsS3Dal dal = new AwsS3Dal( "s3://synapse-dal", parts[0], parts[1] );

        IEnumerable<string> allPlans = dal.GetPlanList();

        IEnumerable<string> somePlans = dal.GetPlanList( "RunAs" );

        IEnumerable<long> iids = dal.GetPlanInstanceIdList( "samplePs1Dyn" );

        Plan p = dal.GetPlan( "samplePs1Dyn" );

        Plan pi = dal.CreatePlanInstance( "samplePs1Dyn" );

        Plan pli = dal.GetPlanStatus( "samplePs1Dyn", 636726881008725363 );
